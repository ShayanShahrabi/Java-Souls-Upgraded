...
public class App{
    ...
    private static Monster chooseRandomAliveMonster(){
        ArrayList<Monster> theAvailableMonsters = getAliveMonsters();
        if(theAvailableMonsters.isEmpty()){
            return null;
        }
        Random rand = new Random();
        return theAvailableMonsters.get(rand.nextInt(theAvailableMonsters.size()));
    }
    //-----------------------------------------------------------------------------
    private static ArrayList<Monster> getAliveMonsters(){
        ArrayList<Monster> theAvailableMonsters = new ArrayList<>();
        for(Monster monster : monsterList){
            if(monster.isAlive){
                theAvailableMonsters.add(monster);
            }
        }
        return theAvailableMonsters;
    }
    //-----------------------------------------------------------------------------
    private static Player chooseRandomAlivePlayer(){
        ArrayList<Player> theAvailablePlayers = new ArrayList<>();
        for(Player player : playerList){
            if(player.isAlive){
                theAvailablePlayers.add(player);
            }
        }
        Random rand = new Random();
        if(theAvailablePlayers.isEmpty()){
            return null;
        }
        return theAvailablePlayers.get(rand.nextInt(theAvailablePlayers.size()));
    }
    //-----------------------------------------------------------------------------
    ...
    private static Player playerNextMove(){
        ...
        if(player instanceof Wizard){
            ...
            int theChoice = choosingScanner.nextInt();
            if(theChoice == 1){
